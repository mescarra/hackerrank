...
abstract sealed class BST[+A <% Ordered[A]] {
  ...
  def add[B >: A <% Ordered[B]](x: B): BST[B] =
    if (isEmpty) BST.make(x)
    else if (x < value) BST.make(value, left.add(x), right)
    else if (x > value) BST.make(value, left, right.add(x))
    else this
  ...
  def remove[B >: A <% Ordered[B]](x: B): BST[B] =
    if (isEmpty) fail("Can't find " + x + " in this tree.")
      ...
      if (left.isEmpty && right.isEmpty) BST.empty
        ...
        val succ = right.min
        BST.make(succ, left, right.remove(succ))
      }
    }
  ...
  def invert[B >: A](implicit num: Numeric[B]): BST[B] =
    if (isEmpty) BST.empty
    else BST.make(num.negate(value), right.invert(num), left.invert(num))
  ...
  def predecessor[B >: A <% Ordered[B]](x: B): A = {
    def forward(t: BST[A], p: List[BST[A]]): A =
      if (t.isEmpty) backward(t, p)//fail("Can't find " + x + " in this tree.")
      else if (x < t.value) {
        if (t.left.isEmpty) backward(t, p)
        else forward(t.left, t :: p)
      } else if (x > t.value) {
        if (t.right.isEmpty) t.value
        else forward(t.right, t :: p)
      }
      else t.value

    def backward(t: BST[A], p: List[BST[A]]): A =
      if (p.isEmpty) x.asInstanceOf[A]
...
object BST {
  ...
  /**
    ...
    * Space - O(1)
    */
  def make[A <% Ordered[A]](x: A, l: BST[A] = Leaf, r: BST[A] = Leaf): BST[A] =
  ...
  /**
    ...
    * Space - O(log n)
    */
  def apply[A <% Ordered[A]](xs: A*): BST[A] = {
  ...
  /**
    ...
    * Space - O(log n)
    */
  def fromSortedArray[A <% Ordered[A]](a: Array[A]): BST[A] = {
  ...
  /**
    ...
    * Space - O(log n)
    */
  def fromSortedList[A <% Ordered[A]](l: List[A]): BST[A] = {
  ...
  /**
    ...
    * Space - O(log n)
    */
  def complete[A <% Ordered[A]](x: A, d: Int): BST[A] =
    if (d == 0) BST.make(x)
    else {
  ...
  /**
    ...
    * Space - O(log n)
    */
  def balanced[A <% Ordered[A]](x: A, s: Int): BST[A] = {
